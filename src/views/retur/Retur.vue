...
<template>
  <div>
    ...
    <el-table :data="tableDate" stripe style="width: 100%;" row-key="id">
      ...
      <el-table-column label="实际归还日期" prop="returnDate"></el-table-column>
...
export default {
  ...
  this.formAdd.nums=res.data.nums
    ...
    BorrowDetail(row) {
      ...
      request.get("/Borrow/listRetur/" + row.id).then(res => {
        this.gridData = res.data;
      });
    },
   
    load() {
      request.get("/Borrow/pageRetur", { params: this.params }).then(res => {
        if (res.code == "200") {
